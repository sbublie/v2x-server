...
class UdpService():
    ...
    def __init__(self):
        ...
        self.foo = asn1tools.compile_files("completeasn.asn", 'uper')

        self.map_responses = []
        self.spat_responses = []

    def handle_mapem(self, data):
        print('decode map')
        # TODO: Decode Header first?
        decoded = False
        try:
            decoded = self.foo.decode('MAPEM', data[data.index(const.MAPEM_IDENTIFIER):])
    ...
    def handle_spatem(self, data):
        print('decode spat')
        # TODO: Decode header first?

        decoded = False
        try:
            decoded = self.foo.decode('SPATEM', data[data.index(const.SPATEM_IDENTIFIER):])
        except Exception as error:
            pass
